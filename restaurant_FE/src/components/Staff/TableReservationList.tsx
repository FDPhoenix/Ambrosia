...
import styles from "../../css/StaffCss/TableReservationList.module.css";
...
const TableReservationList = () => {
    ...
    const statusStyles = {
        ...
        Unknown: { backgroundColor: "#ffcc80", color: "#8b4500" }
    };
                            ...
                            <div className={styles.modalInfoContainer}>
                                ...
                                {selectedBooking.tableId ? (
                                    <div >
                                        ...
                                        <div className={styles.modalInfoRow}>
                                            <strong>Capacity:</strong> <span>{selectedBooking.tableId.capacity}</span>
                                        </div>
                                    </div>
                                ) : (
                                    <div className={styles.modalInfoRow}>
                                    ...
                                    <tbody>
                                        {selectedBooking.dishes.map((dish, index) => (
                                            <tr key={index}>
                                                ...
                                                <td>{dish.dishId.price.toLocaleString()} VND</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                    <tfoot>
                                        <tr className={styles.totalRow}>
                                            <td colSpan={3}>
                                                <div className={styles.totalContainer}>
                                                    <span className={styles.totalLabel}><strong>Total (Before Tax):</strong></span>
                                                    <span className={styles.totalAmount}>
                                                        <strong>
                                                            {selectedBooking.dishes
                                                                .reduce((sum, dish) => sum + dish.dishId.price * dish.quantity, 0)
                                                                .toLocaleString()} VND
                                                        </strong>
                                                    </span>
                                                </div>
            ...
            <Modal isOpen={filterModalOpen} onRequestClose={() => setFilterModalOpen(false)} className={styles.modal} overlayClassName={styles.overlay} shouldCloseOnOverlayClick={false} ariaHideApp={false}>
                <div className={styles.modalContent}>
                    ...
                    <div className={styles.formFields}>
                        ...
                        <label>Status</label>
                        <select value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })} className={styles.formFilter}>
                            <option value="">All</option>
                            <option value="Confirmed">Confirmed</option>
                            <option value="Pending">Pending</option>
                            <option value="Canceled">Canceled</option>
