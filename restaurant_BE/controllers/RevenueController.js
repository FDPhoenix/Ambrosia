  ...
  try {
    ...
    if (day) {
      ...
      const revenue = await Order.aggregate([
        {
          $match: {
            paymentStatus: "Success", // Corrected paymentStatus
    ...
    // Get by month
      ...
      const revenueByDay = await Order.aggregate([
        {
          $match: {
            paymentStatus: "Success", // Ensure correct status data
            createdAt: { $gte: startDate, $lte: endDate },
          },
        },
        {
...
exports.printBill = async (req, res) => {
  try {
    const { id } = req.params;
    const { fields } = req.body; // get directly from body

    // If client sends fields, use them, otherwise print all
